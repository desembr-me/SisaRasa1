<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-[var(--ink)] leading-tight">Listing Baru</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-[var(--paper-card)] border border-[var(--line)] rounded-lg p-6">
                <form method="POST" action="{{ route('mitra.listings.store') }}" class="space-y-4" x-data="{ priceType: '{{ old('price_type', 'gratis') }}' }">
                    @csrf

                    <div>
                        <x-input-label for="title" value="Nama item" />
                        <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title')" placeholder="mis. Roti tawar sisa" required autofocus />
                    ...
                    <div>
                        ...
                        <x-text-input id="quantity" name="quantity" type="number" min="1" max="1000" class="mt-1 block w-full" :value="old('quantity')" required />
                    ...
                    <div x-show="priceType === 'diskon'" class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="original_price" name="original_price" type="number" min="0" class="mt-1 block w-full" :value="old('original_price')" />
                        </div>
                        <div>
                            <x-input-label for="discounted_price" value="Harga diskon (Rp)" />
                            <x-text-input id="discounted_price" name="discounted_price" type="number" min="0" class="mt-1 block w-full" :value="old('discounted_price')" />
                        </div>
                    </div>

                    <div>
                        <x-input-label for="estimated_kg" value="Estimasi kg per porsi" />
                        <x-text-input id="estimated_kg" name="estimated_kg" type="number" step="0.1" min="0.1" max="100" class="mt-1 block w-full" :value="old('estimated_kg', '0.3')" required />
                        <x-input-error :messages="$errors->get('estimated_kg')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="expires_at" value="Berlaku sampai" />
                        <x-text-input id="expires_at" name="expires_at" type="datetime-local" class="mt-1 block w-full" :value="old('expires_at')" required />
                        <x-input-error :messages="$errors->get('expires_at')" class="mt-2" />
                    </div>

                    <div class="flex items-center gap-3 pt-2">
                        <x-primary-button>Simpan Listing</x-primary-button>
